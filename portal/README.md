...
The public `/commands` page lists verified bot commands and FAQ entries maintained at `/admin/knowledge` by Moderator/Admin. The public `/support` form creates private tickets with rate limiting, a honeypot, and an unpredictable reference. A visitor can check status and the staff reply using the reference plus their email; Support and higher roles review tickets under `/admin/tickets`. The existing `/appeal/` form remains the ban appeal route and uses the existing `discord_ban_appeals` table, CSRF, rate limits and Turnstile when configured. Public `/bans` links to it. The landing continues to use the configured Discord guild and Server Widget for visible member avatars.

The portal uses Eloquent for its new content and ticket models, Livewire 3 for refreshed dashboard guild metrics, Spatie Laravel Permission 6 for local Staff role records synchronized from Discord, and Bootstrap 5.3.8 for responsive new public pages. Discord's live guild role remains authoritative; a stale local role cannot grant access. After uploading the **full FTP archive** over an existing installation, preserve `.env` and private storage, back up the database, then sign in as Owner and use **Owner settings → Приложи таблиците**. This runs the pending migrations for tickets, knowledge content and permission tables. The code-only CMS updater cannot install new Composer dependencies or migrations, so it is not suitable for this upgrade. Bootstrap 5.3.8 CSS/JS is copied from the Composer package into the FTP archive; portal.css defines the product colors and typography.
