...
For FTP without server-side Composer, download the `bggamer-ftp-install` artifact from a successful **Build FTP installer** GitHub Actions run. Unpack its ZIP into the FTP web root and remove `portal/bootstrap/cache/config.php` if it exists from a previous deployment. The artifact includes production `portal/vendor/` and no `.env` or setup key. Existing `.env` on an installed site must be preserved. An FTP upload does not delete old remote files. This installer archive is separate from the code-only `bggamer-portal.zip` used by the CMS updater.
