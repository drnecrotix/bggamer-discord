...
Owner uses `/login` with email and password if Discord is unavailable. The same page has a Discord button for Staff; the old `/owner/login` redirects there. After password login, `/owner/settings` can link a Discord account through OAuth. The linked Discord identity can then sign in as Owner without needing a guild staff role. Unlink requires the Owner password. Protect both methods with a strong password and normal server backups. Discord linking uses the same OAuth application callback URL as staff login.
