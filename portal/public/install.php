...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (! filter_var($email, FILTER_VALIDATE_EMAIL) || strlen($password) < 14 || strlen($password) > 256) {
            throw new RuntimeException('Check the Owner email and password (14+ characters).');
        }
        if (! $resuming) {
            if (! in_array($driver, ['mysql', 'pgsql'], true) || ! filter_var($url, FILTER_VALIDATE_URL)
                ...
                throw new RuntimeException('Check the URL and database settings.');
            }
        ...
        $kernel->bootstrap();
        if ($resuming && Illuminate\Support\Facades\Schema::hasTable('portal_owners')
            && Illuminate\Support\Facades\DB::table('portal_owners')->exists()) {
            http_response_code(404);
            throw new RuntimeException('An Owner already exists. Installer unavailable.');
        }
        $exitCode = Illuminate\Support\Facades\Artisan::call('migrate', ['--force' => true]);
        if ($exitCode !== 0 || ! Illuminate\Support\Facades\Schema::hasTable('portal_owners')) {
            throw new RuntimeException('Database migration failed. Check the PHP error log and database permissions, then retry with the same setup code.');
        }
